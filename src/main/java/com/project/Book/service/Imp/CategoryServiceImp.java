...
public class CategoryServiceImp implements CategoryService {
    ...
    @Override
    public CategoryResponse createCategory(CategoryRequest categoryRequest, MultipartFile cateImg) {
        ...
        Category category = categoryMapper.requestDtoToEntity(categoryRequest);
        if(cateImg==null||cateImg.isEmpty()){
            throw new AppException("error.img.notfound", HttpStatus.NOT_FOUND);
        }
        category.setCateImg(UtilClass.saveImgAndGetImgPath(cateImg, FilePath.CATE_IMG.getImgPath()));
        return categoryMapper.entityToResponseDTO(categoryRepository.save(category));
    }

    @Override
    public CategoryResponse updateCategory(int cateId, CategoryUpdateRequest categoryUpdateRequest, MultipartFile cateImg) {
